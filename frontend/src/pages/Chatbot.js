    ...
    try {
      ...
      if (response.data.secure_url) {
        ...
        const options = {
          ...
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            query: `I am suffering from ${result}. Suggest treatment and preventive measures`,
          }),
        };

        const modelResult = await fetch(
    ...
    try {
      ...
      if (pdfUrl) {
        ...
        const response = await fetch(
          "http://localhost:5000/download_pdf",
          PDFOptions
        );
        ...
        if (result == 1) {
          ...
          const res = await fetch(
            "http://localhost:5000/pdf_answer",
            requestOptions
          );
    ...
    <div className="app mt-20">
      ...
      <div className="spotlight__wrapper">
        ...
        <div className="Image">
          ...
          <button className="upload" onClick={uploadFileToCloudinary}>
            Upload
          </button>
        </div>

        <b>Upload a PDF file</b>
        <br></br>
        <div className="PDF">
          <input type="file" accept=".pdf" onChange={handlePDFFileChange} />
          <button className="upload" onClick={uploadPDFFileToCloudinary}>
            Upload
          </button>
        </div>
